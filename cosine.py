...
pygame.display.set_caption("3D Cosine Wave - 45° Side View (Interactive)")
...
while running:
    for event in pygame.event.get():
        ...
        if event.type == pygame.KEYDOWN:
            if event.key == pygame.K_ESCAPE or event.key == pygame.K_q:
                ...
                paused = not paused
            elif event.key == pygame.K_r:  # Reset phase
                phase = 0
    ...
    # Draw 3D cosine wave points
    for i in range(points_count):
        z = depth_start + (i / points_count) * (depth_end - depth_start)

        # Horizontal + vertical oscillation (both cosine-based)
        x = math.cos(frequency * z + phase) * horizontal_amplitude
        y = math.cos(frequency * z + phase + math.pi / 2) * vertical_amplitude
