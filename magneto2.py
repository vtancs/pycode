...
pygame.display.set_caption("Magnetospheres of Earth and Neptune (solar storms)")
...
# Solar wind particles (start with 200)
...
# Load textures
earth_base = pygame.image.load("earth.jpg").convert_alpha()
neptune_base = pygame.image.load("neptune.jpg").convert_alpha()
earth_base = pygame.transform.smoothscale(earth_base, (earth_radius * 2, earth_radius * 2))
neptune_base = pygame.transform.smoothscale(neptune_base, (neptune_radius * 2, neptune_radius * 2))
...
def draw_rotating_planet(center, base_img, radius, angle):
    rotated = pygame.transform.rotate(base_img, angle)
    scaled = pygame.transform.smoothscale(rotated, (radius * 2, radius * 2))
    rect = scaled.get_rect(center=center)
    screen.blit(scaled, rect)

def draw_planet_with_magnetosphere(center, base_img, planet_radius, magnetosphere_radius,
                                   angle, compression_factor=1.0, field_strength=1.0,
    ...
    draw_rotating_planet(center, base_img, planet_radius, angle)

    return left_radius, right_radius, magnetosphere_radius, field_strength

def update_solar_wind(planet_data, speed):
    for p in particles:
        p["x"] += speed
...
def update_solar_wind_conditions():
    """Randomly vary solar wind speed and density to simulate storms"""
    global solar_wind_speed, solar_wind_density, particles
    t = frame_count / 200
    base_speed = 4 + 2 * math.sin(t)
    base_density = 200 + int(100 * math.sin(t))

    # Occasionally trigger strong solar storms
    if random.random() < 0.002:
        solar_wind_speed = random.randint(7, 10)
        solar_wind_density = random.randint(300, 400)
    else:
        solar_wind_speed = int(base_speed)
        solar_wind_density = base_density

    # Adjust particle list size
    if len(particles) < solar_wind_density:
        for _ in range(solar_wind_density - len(particles)):
            particles.append({"x": random.randint(-200, WIDTH), "y": random.randint(0, HEIGHT)})
    elif len(particles) > solar_wind_density:
        particles = particles[:solar_wind_density]

def main():
    global phase, frame_count, earth_angle, neptune_angle
    running = True
    while running:
        frame_count += 1
        draw_background()
        ...
        neptune_angle = (neptune_angle + 0.05) % 360

        # Update solar wind dynamics
        update_solar_wind_conditions()

        # Compression reacts to solar wind (faster wind → more compressed)
        compression = min(1.5, 0.5 + solar_wind_speed / 6)

        # Earth
        earth_left, earth_right, earth_vert, earth_strength = draw_planet_with_magnetosphere(
            earth_pos, earth_base, earth_radius, earth_magnetosphere,
            earth_angle, compression_factor=compression, field_strength=1.0,
        ...
        neptune_left, neptune_right, neptune_vert, neptune_strength = draw_planet_with_magnetosphere(
            neptune_pos, neptune_base, neptune_radius, neptune_magnetosphere,
            neptune_angle, compression_factor=compression * 0.2, field_strength=0.5,
        ...
        screen.blit(font.render(f"Earth", True, WHITE),
                    (earth_pos[0] - 40, earth_pos[1] + earth_radius + 20))
        screen.blit(font.render(f"Neptune", True, WHITE),
                    (neptune_pos[0] - 60, neptune_pos[1] + neptune_radius + 20))
        screen.blit(font.render(f"Solar Wind Speed: {solar_wind_speed}", True, WHITE), (20, 20))
