"""
Order Book Simulator with Pygame Visualization and Manual Orders
- Bids = green bars, Asks = red bars
- Trades = yellow text log
- Press B to place a manual BUY order (enter price/qty in console)
- Press S to place a manual SELL order
- Press R to toggle random order flow on/off
...
class Visualizer:
    ...
    def draw(self):
        ...
        # Show toggle status
        toggle_text = f"Random flow: {'ON' if self.random_flow else 'OFF'}"
        self.screen.blit(self.font.render(toggle_text,True,(200,200,200)),(self.width-200,10))
        ...
        while running:
            for event in pygame.event.get():
                ...
                if event.type == pygame.KEYDOWN:
                    ...
                    if event.key == pygame.K_r: self.random_flow = not self.random_flow
                    if event.key == pygame.K_b:
                        try:
                            price = float(input("Enter BUY price: "))
                            qty = int(input("Enter BUY quantity: "))
                        except:
                            price = round(random.uniform(90,110),2)
                            qty = random.randint(1,50)
                            print(f"Defaulting to BUY {qty}@{price}")
                    ...
                    if event.key == pygame.K_s:
                        try:
                            ...
                            qty = int(input("Enter SELL quantity: "))
                        except:
                            price = round(random.uniform(90,110),2)
                            qty = random.randint(1,50)
                            print(f"Defaulting to SELL {qty}@{price}")
            ...
            self.draw()
            self.clock.tick(2) # updates per second
        pygame.quit()
